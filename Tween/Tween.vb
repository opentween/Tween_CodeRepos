...
Public Class TweenMain
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        For idx = 0 To _section.ListElement.Count - 1
            ...
            If name <> "Recent" And name <> "Reply" And name <> "Direct" Then
                ...
                myTab.listCustom = New Tween.TweenCustomControl.DetailsListView
        ...
        For cnt = 0 To tlList.Count - 1
            ...
            For Each ts As TabStructure In _tabs
                ...
                If hit Then
                    ...
                    ts.listCustom.Items.Add(lvItem.Clone)
                                ...
                                If rslt.sIds.Contains(itm.SubItems(5).Text) Then
                                    ...
                                    For idx As Integer = 0 To ListTab.TabCount - 1
                                        If ListTab.TabPages(idx).Text <> rslt.tName And ListTab.TabPages(idx).Text <> "Direct" Then
                                            Dim MyList2 As TweenCustomControl.DetailsListView = CType(ListTab.TabPages(idx).Controls(0), Tween.TweenCustomControl.DetailsListView)
    ...
    Private Sub MyList_ColumnClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ColumnClickEventArgs) Handles Timeline.ColumnClick, Reply.ColumnClick, DirectMsg.ColumnClick
        ...
        For Each _tab As TabPage In ListTab.TabPages
            ...
            Dim MyList As Tween.TweenCustomControl.DetailsListView = CType(_tab.Controls(0), Tween.TweenCustomControl.DetailsListView)
    ...
    Private Sub ReadedStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ReadedStripMenuItem.Click
        ...
        Dim umng As Boolean
        For Each ts As TabStructure In _tabs
            If ts.tabName = ListTab.SelectedTab.Text Then
                umng = ts.unreadManage
                Exit For
            End If
        Next
        If MyList.SelectedItems.Count > 0 Then
            For Each lItem As ListViewItem In MyList.SelectedItems
                'lItem.Font = New Font(lItem.Font, FontStyle.Regular)
                If lItem.SubItems(8).Text = "False" Then
                    lItem.SubItems(8).Text = "True"
                    lItem.Font = _fntReaded
                    lItem.ForeColor = _clReaded
                    If lItem.SubItems(10).Text = "True" And SettingDialog.OneWayLove Then lItem.ForeColor = _clOWL
                    If lItem.SubItems(9).Text = "True" Then lItem.ForeColor = _clFav
                    If MyList.Name <> "DirectMsg" Then
                        For i As Integer = 0 To ListTab.TabCount - 1
                            If i <> ListTab.SelectedIndex And ListTab.TabPages(i).Text <> "Direct" Then
                                Dim tmpumng As Boolean
                                For Each ts As TabStructure In _tabs
                                    If ts.tabName = ListTab.TabPages(i).Text Then
                                        tmpumng = ts.unreadManage
                                        Exit For
                                    End If
                                Next
                                Dim tmpList As Tween.TweenCustomControl.DetailsListView = CType(ListTab.TabPages(i).Controls(0), Tween.TweenCustomControl.DetailsListView)
                                Dim ur As Boolean = False
                                For Each tmpItem As ListViewItem In tmpList.Items
                                    If tmpItem.SubItems(4).Text = lItem.SubItems(4).Text And _
                                       tmpItem.SubItems(5).Text = lItem.SubItems(5).Text Then
                                        tmpItem.SubItems(8).Text = "True"
                                        tmpItem.Font = _fntReaded
                                        tmpItem.ForeColor = _clReaded
                                        If tmpItem.SubItems(10).Text = "True" And SettingDialog.OneWayLove Then tmpItem.ForeColor = _clOWL
                                        If tmpItem.SubItems(9).Text = "True" Then tmpItem.ForeColor = _clFav
                                    Else
                                        If ur = False AndAlso tmpItem.SubItems(8).Text = "False" Then
                                    ...
                                    End If
                                Next
                                If ur And tmpumng And SettingDialog.UnreadManage Then
                                    'ListTab.TabPages(i).ImageIndex = 0
                                Else
                                    ListTab.TabPages(i).ImageIndex = -1
                                End If
                            End If
                        Next
                    End If
        ...
        If umng And SettingDialog.UnreadManage Then
            For Each tmpItem As ListViewItem In MyList.Items
                If tmpItem.SubItems(8).Text = "False" Then
                    'ListTab.SelectedTab.ImageIndex = 0
                    Exit Sub
                End If
            Next
        End If
        ListTab.SelectedTab.ImageIndex = -1
    End Sub
    ...
    Private Sub UnreadStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UnreadStripMenuItem.Click
        ...
        If MyList.SelectedItems.Count > 0 Then
            For Each lItem As ListViewItem In MyList.SelectedItems
                'lItem.Font = New Font(lItem.Font, FontStyle.Bold Or FontStyle.Underline)
                If lItem.SubItems(8).Text = "True" Then
                    lItem.SubItems(8).Text = "False"
                    If umng And SettingDialog.UnreadManage Then
                        lItem.Font = _fntUnread
                        lItem.ForeColor = _clUnread
                    End If
                    If lItem.SubItems(10).Text = "True" And SettingDialog.OneWayLove Then lItem.ForeColor = _clOWL
                    If lItem.SubItems(9).Text = "True" Then lItem.ForeColor = _clFav
                    If MyList.Name <> "DirectMsg" Then
                        For i As Integer = 0 To ListTab.TabCount - 1
                            If i <> ListTab.SelectedIndex And ListTab.TabPages(i).Text <> "Direct" Then
                                ...
                                Dim ur As Boolean = False
                                For Each tmpItem As ListViewItem In tmpList.Items
                                    If tmpItem.SubItems(5).Text = lItem.SubItems(5).Text Then
                                        tmpItem.SubItems(8).Text = "False"
                                        If tmpUmng And SettingDialog.UnreadManage Then
                                            tmpItem.Font = _fntUnread
                                            tmpItem.ForeColor = _clUnread
                                        End If
                                        If tmpItem.SubItems(10).Text = "True" And SettingDialog.OneWayLove Then tmpItem.ForeColor = _clOWL
                                        If tmpItem.SubItems(9).Text = "True" Then tmpItem.ForeColor = _clFav
                                        ur = True
        ...
        If umng And SettingDialog.UnreadManage Then ListTab.SelectedTab.ImageIndex = 0
    End Sub
    ...
    Private Sub SettingStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SettingStripMenuItem.Click
        ...
        If SettingDialog.ShowDialog() = Windows.Forms.DialogResult.OK Then
            ...
            If isz <> SettingDialog.IconSz Then
                ...
                Dim idx As Integer = 0
                For idx = 0 To ListTab.TabCount - 1
                    Dim myList As Tween.TweenCustomControl.DetailsListView = CType(ListTab.TabPages(idx).Controls(0), Tween.TweenCustomControl.DetailsListView)
                    myList.SmallImageList = TIconSmallList
                Next

                Dim myList2 As Tween.TweenCustomControl.DetailsListView = CType(ListTab.SelectedTab.Controls(0), Tween.TweenCustomControl.DetailsListView)
                ...
                CType(ListTab.SelectedTab.Controls(0), Tween.TweenCustomControl.DetailsListView).Refresh()
            ...
            If SettingDialog.OneWayLove = True Then
                For Each myTab As TabPage In ListTab.TabPages
                    If myTab.Text <> "Direct" Then
                        Dim myList As Tween.TweenCustomControl.DetailsListView = CType(myTab.Controls(0), Tween.TweenCustomControl.DetailsListView)
            ...
            For Each myTab As TabPage In ListTab.TabPages
                Dim myList As Tween.TweenCustomControl.DetailsListView = CType(myTab.Controls(0), Tween.TweenCustomControl.DetailsListView)
                Dim umng As Boolean = False
                For Each ts As TabStructure In _tabs
                    If ts.tabName = myTab.Text Then
                        umng = ts.unreadManage
                        Exit For
                    End If
                Next
                For Each myItem As ListViewItem In myList.Items
                    If SettingDialog.UnreadManage = True And umng Then
                    ...
                    Else
                        ...
                        myItem.Font = _fntReaded
                    End If
                    If SettingDialog.OneWayLove Or myTab.Text = "Direct" Then
                        If myItem.SubItems(10).Text = "True" Then myItem.ForeColor = _clOWL
                    End If
                    If myItem.SubItems(9).Text = "True" Then myItem.ForeColor = _clFav
                Next
            Next
            'ColorizeList(False)
            TimerColorize.Start()
    ...
    Private Sub ListTab_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ListTab.SelectedIndexChanged
        ...
        '新しく表示されたリストを再描画
        Dim MyList As Tween.TweenCustomControl.DetailsListView = CType(ListTab.SelectedTab.Controls(0), TweenCustomControl.DetailsListView)
        MyList.Update()
        TimerColorize.Start()
        '新しいタブ名を保管
    ...
    Private Sub SetListProperty()
        '直前のリスト特定
        Dim MyList As Tween.TweenCustomControl.DetailsListView = Nothing
        For Each myTab As TabPage In ListTab.TabPages
        ...
        For Each _tab As TabPage In ListTab.TabPages
            If _tab.Text <> _curTabText Then
                Dim lst As Tween.TweenCustomControl.DetailsListView = CType(_tab.Controls(0), Tween.TweenCustomControl.DetailsListView)
    ...
    Private Sub MyList_DrawItem(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DrawListViewItemEventArgs) Handles Timeline.DrawItem, Reply.DrawItem, DirectMsg.DrawItem
        'e.DrawDefault = True
        If SettingDialog.IconSz = Setting.IconSizes.Icon48 Or _
           SettingDialog.IconSz = Setting.IconSizes.Icon24 Then
            ...
            'アイコンカラム位置取得
            For cnt = 0 To 4
                If MyList.Columns(cnt).Text = "" Then
            ...
            Dim rct As New Rectangle(x, e.Item.SubItems(idx).Bounds.Y + 1, wd, MyList.SmallImageList.ImageSize.Height)
            ...
            For i As Integer = 0 To ListTab.TabPages.Count - 1
                If ListTab.TabPages(i).Text = _ntnm Then
                    ...
                    Else
                        i += 1
                    End If
                    myList = CType(ListTab.TabPages(i).Controls(0), Tween.TweenCustomControl.DetailsListView)
...
RETRY:
    ...
    Private Sub ReFilter()
        ...
        For Each _tab As TabPage In ListTab.TabPages
            If _tab.Text <> "Recent" And _tab.Text <> "Reply" And _tab.Text <> "Direct" Then
        ...
        For Each itm As ListViewItem In Timeline.Items
            ...
            For Each ts As TabStructure In _tabs
                ...
                Next
                If hit Then
                    ts.listCustom.Items.Add(itm.Clone)
                End If
            Next
            If mv = False Then
    ...
    Private Sub ClearTabMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ClearTabMenuItem.Click
        ...
        For Each tb As TabPage In ListTab.TabPages
            If tb.Text = _rclickTabName Then
                Dim myList As TweenCustomControl.DetailsListView = CType(tb.Controls(0), TweenCustomControl.DetailsListView)
                myList.Items.Clear()
                tb.ImageIndex = -1
                Exit For
            End If
...
Public Class TabStructure
    ...
    Public listCustom As Tween.TweenCustomControl.DetailsListView
